...
## Introduction
Well, let we make a recap of what we have done since now, we have build a [bind shell](https://bolonobolo.github.io/SLAE32-1/), a [reverse shell](https://bolonobolo.github.io/SLAE32-2/) and a [egghunter shellcode](https://bolonobolo.github.io/SLAE32-3/). A lots of nice stuff for hackers out there who want to test their skills, but it's a useless effort if we want to test thing nice things a real scenario where we can encoter Antivirus or IDS systems. Fortunatly some techniques comes in our help.
...
print "Encoded Shellcode Len: %d" % len(bytearray(encoded))
print encoded + end
db = str(encoded).replace("\\x", ",0x")
db = db[1:]
# end string char is 0xaa
print "NASM version: \n" + db + str(end).replace("\\x", ",0x")
```
running the script we obtain an obfuscated shellcode, much larger that than the original, but hey a small but full visible shellcode have no future.

```bash
root@slae32-lab:/home/bolo/SLAE32/assignements/4_encoder# ./encoder_mixer.py 
Encoded shellcode ...
Shellcode Len: 25
