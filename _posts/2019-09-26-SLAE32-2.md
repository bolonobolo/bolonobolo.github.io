...
Has said before we have already learnd how to build a bind shellcode in the [previous post](https://bolonobolo.github.io/SLAE32-1/), but i think a review couldn't damage us, so let's (re)start the fun part.
...
```| 0 (unused char) | IP address (192.168.1.223) | port in network byte order | 0x02 (AF_INET value) |```
