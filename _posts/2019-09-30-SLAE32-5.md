...
So in EAX must bo stored the value of the syscall (0x5), in EBX must be stored the address of the pathname and in ECX in must be stored the permissions 0x401 that set the gid bit previously setted to 0
In octal 0x401 is 2001, ```man 7 inod``` give us explanation
```bash
# printf "%o\n" 0x401
2001
```
