...
- EBX register contains the pointer to the ```filename``` should be executed, in our case ```/bin//sh``` with 2 slashes because we want a value divisible by 4, the string ```/bin//sh``` as also to be reverted because we are working in little endian environment. Because we are wor
