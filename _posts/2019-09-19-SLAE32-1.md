...
This is also the first assignment from Viviek Ramchandran's SLAE32 course.<br>
...
It works like a charm, so now we can extract the hexadecimal value of the shellcode with an esotheric objdump coomand picked up from [CommandLineFu](https://www.commandlinefu.com/commands/view/6051/get-all-shellcode-on-binary-file-from-objdump).
```bash
...
Good job, now it's time to add the power to select the port where the bind shell will run.
...
## TECH NOTES
The NULL byte is not the only one bad character that shouldn't be inserted in our shellcode. Accordingly with [Infosec Institute](https://resources.infosecinstitute.com/stack-based-buffer-overflow-in-win-32-platform-part-6-dealing-with-bad-characters-jmp-instruction/) the list of badchars are:
- 00 for NULL
- 0A for Line Feed \n
