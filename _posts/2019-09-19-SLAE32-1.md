...
- EBX register contains the pointer to the ```filename``` that should be executed, in our case ```/bin//sh``` with 2 slashes because we want a value divisible by 4, the string ```/bin//sh``` as also to be reverted because we are working in little endian environment. Because we are wor
- ECX contains a pointer to ```argv``` that is an array of argument strings passed to the new program, in our case is the address of the filename to execute
...
; root@slae32-lab:/home/bolo/SLAE32/shellcode# ./reverse_command.py "/bin//sh"
; hs// : 68732f2f
; nib/ : 6e69622f
; String length : 8
; Hex length : 16
