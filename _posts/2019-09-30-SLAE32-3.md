...
Let's review what we need to code:
1. build an egg of 8 bytes (4 bytes repeated) and store it in a register
2. execute the syscall 
3. compare the return code with -14 (0x0f in hex)<br>
if it is EFAULT (14), ZF is set then jump to the next memory PAGE SIZE because it is a invalid memory address<br>
or compare the value with the egg
