...
if it is EFAULT (14), ZF is set then jump to the next memory PAGE SIZE because it is a invalid memory address otherwise compare the value with the egg
4. if the comparison doesn't match step into the next address and repeat from point 2<br>
5. if ZF is set, comparison match, th egg is found and we can execute our shellcode.
...
Now copy the shellcode in a C program that execute it and give us the length of the hunter and the shellcode
