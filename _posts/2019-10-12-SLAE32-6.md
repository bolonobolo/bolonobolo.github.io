...
A very interesting shellcode, it's a ```/bin/sh``` but very very small, only 21 bytes. 
```C
/*
...
```
It's very minimal, perfect for some polyphormism adjustment. We know that our shellcode can't exite the 150% of the orignal one, in this case starting from a shellcode lencht of 21 bytes we must write another one at most of 31 bytes rounded down. Let's start changing the xoring operations, ```cdq``` convert Doubleword to Quadword and store the result in EDX (0x0), next we ```mul edx``` and store the result in EAX (0x0), last we load the value of EAX in ECX (0x0), now we can change the ```push hs/nib//``` operations in ```mov``` to some registers and then ```push``` the registers in the stack, last we can ```lea``` the stack pointer to EBX and call the ```execve``` syscall. Here's the assembly code.
...
int 0x80 
```
We can load the code in our C executer and check the lenght 
```bash
