...
This is the default ```execve``` nasm file but you can use every shellcode you want
```nasm
...
```
Put the shellcode inside the ```python``` script
...
```
Load the encoded shellcode inside the decoder
```nasm
#################### decoder_mixer.asm ############################
...
```
Compile the assembly, get the shellcode and put it inside the C executer
```bash
############################### shellcode ############################
